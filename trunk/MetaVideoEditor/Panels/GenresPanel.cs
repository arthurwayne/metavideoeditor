...
{
    public partial class GenresPanel : BasePanel
    {
        public GenresPanel()
        {
            InitializeComponent();
            label1.Text = Kernel.Instance.GetString("AddGenreGenTab");
            label2.Text = Kernel.Instance.GetString("NoGenreGenTab");
            MainPanel = panel1;
        }
        ...
        public override void UpdateData()
        {
            ...
            if (SelectedItem != null)
            {
                if (SelectedItem.Genres.IsNonEmpty())
                {
                    ...
                    int index = 0;
                    foreach (string text in SelectedItem.Genres)
                    {
        ...
        void panel_OnTextDelete(object sender, EventArgs e)
        {
            ...
            SelectedItem.Genres.RemoveAll(a => a == ap.text);
            HasChanged();
            UpdateData();
        }
        ...
        {
            TextPanel ap = sender as TextPanel;
            string genre = SelectedItem.Genres.Find(a => a == ap.text);
        ...
        void panel_OnMouseMoveEvent(object sender, MouseEventArgs e)
        {
            ...
            if (drag)
            {
                ...
                if (tex.index != index)
                {
                    ...
                    if (ap != null)
                    {
                        string tmp = SelectedItem.Genres[tex.index];
                        SelectedItem.Genres[tex.index] = SelectedItem.Genres[index];
                        SelectedItem.Genres[index] = tmp;
                        ap.index = tex.index;
        ...
        private void AddGenre()
        {
            ...
            {
                if (SelectedItem.Genres == null)
                    SelectedItem.Genres = new List<string>();
                SelectedItem.Genres.Add(textBox.Text);
                textBox.Text = "";
                HasChanged();
                UpdateData();
            }
