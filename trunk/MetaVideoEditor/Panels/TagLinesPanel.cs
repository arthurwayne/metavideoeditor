...
{
    public partial class TagLinesPanel : BasePanel
    {
        public TagLinesPanel()
        {
            InitializeComponent();
            label1.Text = Kernel.Instance.GetString("AddTaglineTagTab");
            label2.Text = Kernel.Instance.GetString("NoTaglineTagTab");
            MainPanel = panel1;
        }
        ...
        void panel_OnTextDelete(object sender, EventArgs e)
        {
            ...
            SelectedItem.TagLines.RemoveAll(a => a == ap.text);
            HasChanged();
            UpdateData();
        }
        ...
        {
            TextPanel ap = sender as TextPanel;
            string genre = SelectedItem.TagLines.Find(a => a == ap.text);
        ...
        void panel_OnMouseMoveEvent(object sender, MouseEventArgs e)
        {
            ...
            if (drag)
            {
                ...
                if (tex.index != index)
                {
                    ...
                    if (ap != null)
                    {
                        string tmp = SelectedItem.TagLines[tex.index];
                        SelectedItem.TagLines[tex.index] = SelectedItem.TagLines[index];
                        SelectedItem.TagLines[index] = tmp;
                        ap.index = tex.index;
        ...
        private void AddTagLine()
        {
            ...
            {
                if (SelectedItem.TagLines == null)
                    SelectedItem.TagLines = new List<string>();
                SelectedItem.TagLines.Add(textBox.Text);
                textBox.Text = "";
                HasChanged();
                UpdateData();
            }
