...
{
    public partial class StudiosPanel : BasePanel
    {
        public StudiosPanel()
        {
            InitializeComponent();
            label1.Text = Kernel.Instance.GetString("AddStudioStTab");
            label2.Text = Kernel.Instance.GetString("NoStudioStTab");
            MainPanel = panel1;
        }
        ...
        public override void UpdateData()
        {
            ...
            if (SelectedItem != null)
            {
                if (SelectedItem.Studios.IsNonEmpty())
                {
                    ...
                    int index = 0;
                    foreach (string text in SelectedItem.Studios)
                    {
        ...
        void panel_OnTextDelete(object sender, EventArgs e)
        {
            ...
            SelectedItem.Studios.RemoveAll(a => a == ap.text);
            HasChanged();
            UpdateData();
        }
        ...
        {
            TextPanel ap = sender as TextPanel;
            string genre = SelectedItem.Studios.Find(a => a == ap.text);
        ...
        void panel_OnMouseMoveEvent(object sender, MouseEventArgs e)
        {
            ...
            if (drag)
            {
                ...
                if (tex.index != index)
                {
                    ...
                    if (ap != null)
                    {
                        string tmp = SelectedItem.Studios[tex.index];
                        SelectedItem.Studios[tex.index] = SelectedItem.Studios[index];
                        SelectedItem.Studios[index] = tmp;
                        ap.index = tex.index;
        ...
        private void AddStudio()
        {
            ...
            {
                if (SelectedItem.Studios == null)
                    SelectedItem.Studios = new List<string>();
                SelectedItem.Studios.Add(textBox.Text);
                textBox.Text = "";
                HasChanged();
                UpdateData();
            }
